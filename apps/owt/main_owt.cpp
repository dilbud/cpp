...
class SessionEchoVisitor
    : public owt::quic::WebTransportSessionInterface::Visitor {
   public:
    explicit SessionEchoVisitor(
        owt::quic::WebTransportSessionInterface *session)
        : session_(session) {}
    ~SessionEchoVisitor() {}
    ...
    void OnConnectionClosed() override { std::printf("Con Closed \n"); }
    void OnIncomingStream(
        owt::quic::WebTransportStreamInterface *stream) override {
    ...
    void OnDatagramReceived(const uint8_t *data, size_t length) override {
        ...
        switch (ms) {
            ...
            case MessageStatus::kEncryptionNotEstablished:
                std::printf(
                    "Failed to send message because encryption is not "
                    "established yet\n");
                break;
            case MessageStatus::kUnsupported:
                std::printf(
                    "Failed to send message because MESSAGE frame is not "
                    "supported by the connection\n");
                break;
            case MessageStatus::kBlocked:
                std::printf(
                    "Failed to send message because connection is congestion "
                    "control blocked or underlying socket is write blocked\n");
                break;
            case MessageStatus::kTooLarge:
                std::printf(
                    "Failed to send message because the message is too large "
                    "to fit into a single packet\n");
                break;
            case MessageStatus::kInternalError:
                std::printf(
                    "Failed to send message because connection reaches an "
                    "invalid state\n");
                break;
            case MessageStatus::kUnavailable:
                std::printf(
                    "Message status is not available. When C++17 std::optional "
                    "is enabled, this value will be removed\n");
                break;
            default:
                std::printf("Unknown\n");
        }
...
class ServerEchoVisitor
   ...
   public:
    ...
    void OnEnded() override { std::printf("Session end\n"); }
...
void quicServer() {
    ...
    owt::quic::WebTransportFactory *webTFactory =
        owt::quic::WebTransportFactory::Create();

    owt::quic::WebTransportServerInterface *webTServerInter =
        webTFactory->CreateWebTransportServer(
            8500, "/workspaces/cpp/cert/out/leaf_cert.p12", "hello");
